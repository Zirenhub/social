...
export default function Sidebar() {
  ...
  return (
    <div ref={sidebarRef} className="overflow-y-scroll flex flex-col gap-3 no-scrollbar h-screen">
      {/* Search Box */}
      <Search />
        ...
        {[
          ...
          >
            <div className="flex items-center">
              ...
              <span className="text-gray-800 dark:text-gray-200 font-medium">{item.topic}</span>
            </div>
            <span className="text-xs text-gray-500 dark:text-gray-400">{item.posts} posts</span>
          </div>
        ))}
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400 px-4 mt-3">
        <div className="flex flex-wrap gap-x-2 gap-y-1 mb-2">
          {["About", "Help Center", "Privacy", "Terms", "Cookies", "Advertising", "Jobs", "Settings"].map(
            (link, index) => (
